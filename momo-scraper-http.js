    ...
    while (attempt < retries) {
        try {
            ...
            return await puppeteer.launch({
                headless: true,
                executablePath: "C:\\chrome-win\\chrome.exe", // Adjust as necessary
                args: [
                    "--disable-gpu",
                    "--no-sandbox",
                    ...
                    "--disable-web-security", // Disable web security (optional, for extreme cases)
                ],
                ignoreHTTPSErrors: true, // This ensures Puppeteer ignores certificate errors
            });
        } catch (error) {
            attempt++;
...
/**
 * Navigate to the desired page with HTTP fallback.
 *
 * This function first tries to navigate to the HTTPS version of the URL. If that fails,
 * it attempts to navigate to the HTTP version. It waits until the page is fully loaded
 * or a timeout occurs. It also waits for a specific element to ensure the target table
...
async function navigateToPage(page) {
    const urls = [
        "https://momoscreener.com/scanner",
        "http://momoscreener.com/scanner",
    ];
    ...
    for (const url of urls) {
        try {
            ...
            if (verbose) console.log(`Successfully navigated to: ${url}`);
            return; // Exit the function on successful navigation
        } catch (error) {
            console.error(`Failed to navigate to: ${url}`, error);
        }
    }

    throw new Error("Navigation failed for both HTTPS and HTTP versions.");
}
