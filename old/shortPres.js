...
const SOUND_COOLDOWN_MS = 3000; // Debounce period (3 seconds)
...
let tickerNewsCounts = new Map(); // Track news counts for each ticker
let newEntriesFlag = false; // Track if new entries have been added
...
const collectAllNews = async (tickers) => {
    let newNews = []; // Store new news items for the current iteration
    ...
    for (const ticker of tickers) {
        ...
        if (newsData && newsData.length > 0) {
            const uniqueHeadlines = new Set(); // Track unique headlines
            ...
            newsData.forEach(newsItem => {
                ...
                if (!processedNews.has(uniqueKey)) {
                    ...
                    if (VERBOSE) console.log(`New headline found for ${ticker}: ${headline}`);
                }
            });

            // Count the news for this ticker
            const headlineCount = uniqueHeadlines.size;
            if (headlineCount > 0) {
                newEntriesFlag = true; // Set the flag for new entries
                const newsType = (headlineCount === 1) ? 'unique' : 'shared';
                
                if (newsType === 'unique') {
                    tickerNewsCounts.set(ticker, (tickerNewsCounts.get(ticker) || 0) + headlineCount);
                } else {
                    tickerNewsCounts.set(ticker, (tickerNewsCounts.get(ticker) || 0));
                }

                await playAlertSound(); // Play alert sound if there's new news
                newNews.push(ticker); // Track tickers with new news
            }
        } else {
...
// Print the ticker news entries in a new table
const printTickerEntries = () => {
    if (VERBOSE) console.log('Preparing to print ticker entries...');

    // Create a new table
    const table = new Table({
        head: ['Ticker', 'News'],
        colWidths: [10, 50], // Adjust widths as needed
    });

    // Populate the table with tickers and their news counts
    currentTickers.forEach(ticker => {
        const count = tickerNewsCounts.get(ticker) || 0;
        const newsSymbols = '*'.repeat(count); // Use asterisks instead of stars
        
        const rowColor = newEntriesFlag ? chalk.bgYellow.black : (ticker) => ticker; // Yellow background with black text
        table.push([rowColor(`${ticker}`), newsSymbols]);
    });

    console.clear();  // Clear the console before printing the table
    console.log(table.toString());
    newEntriesFlag = false; // Reset the new entries flag for the next iteration

    if (VERBOSE) console.log('Ticker entries printed successfully.');
};
...
const processTickers = async (tickersToProcess = currentTickers) => {
    ...
    await collectAllNews(tickersToProcess); // Collect all news
    printTickerEntries(); // Print the updated ticker entries
};
...
const main = async () => {
    ...
    // Start watching the ticker file for changes
    watchFile();
};
